...
最后更新：2026-08-29  
状态：元数据离线审计完成；等待个人 ONC API token 后执行只查询探针。当前不进行批量下载。

## 1. 角色与研究问题

这条路线的首要用途是扩充有 MMSI 身份标注的监督训练/开发数据，并在独立船舶数量增加后复核
20 s 全局注意力是否有效。Belgian AIS 保持为未参与训练的跨水域外部评测，避免同一个外部数据集
同时承担训练扩充和最终验证。
...
必须使用同一 MMSI split、同一 20 s manifest、相同优化预算与选模指标。Belgian AIS 在此阶段仍不
参与训练或超参数选择。
...
- 无法稳定定位：停止回溯，不把整库下载作为补救，转向 Belgian 原始连续性或其他有身份长录音。
