...
if [[ "$MODE" != "smoke" && "$MODE" != "formal" ]]; then
  echo "Usage: bash scripts/train/run_conformer_f0_s2_seed42.sh [smoke|formal]" >&2
  exit 2
fi
...
if [[ "$MODE" == "smoke" ]]; then
  OUTPUT_ROOT="$RUN_ROOT/smoke_f0_s2_vessel_dynamic_20s_seed42"
  LOG_PATH="$LOG_ROOT/smoke_sampling_v1_f0_s2_vessel_dynamic_20s_seed42.log"
  MODE_ARGS=(
    --epochs 1
    --max-train-batches 2
    --max-eval-batches 2
  ...
  OUTPUT_ROOT="$RUN_ROOT/formal_f0_s2_vessel_dynamic_20s_seed42"
  LOG_PATH="$LOG_ROOT/formal_sampling_v1_f0_s2_vessel_dynamic_20s_seed42.log"
  MODE_ARGS=(
    --epochs 8
    --num-workers 4
  )
